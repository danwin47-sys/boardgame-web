        ...
        if is_expansion in ("1", "TRUE", "True", "true"):
            ...
            if storage_mode == STORAGE_MODE_INDEPENDENT:
                ...
                if parent_status == GAME_STATUS_BORROWED:
                    return (
                        True,
                        f"提示：此為擴充，主遊戲「{parent_name}」已被借出",
                        {"parent": parent, "parent_borrowed": True},
                    )
                else:
                    return (
                        True,
                        f"提示：此為擴充，需要主遊戲「{parent_name}」",
                        {"parent": parent, "parent_borrowed": False},
                    )
            else:
                # 合併收納：不應單獨借出
                return (False, f"此擴充為合併收納模式，請借出主遊戲「{parent_name}」", {"parent": parent})
        else:
            # 如果是主遊戲，檢查是否有合併收納的擴充
            ...
            if merged_expansions:
                ...
                return (
                    True,
                    f"將同時借出合併收納的擴充：{', '.join(exp_names_filtered)}",
