...
class GameService:
    ...
    def _get_header_indices(self, header: List[str]) -> Dict[str, int]:
        ...
        try:
            ...
            # Custodian is optional in some versions, but we should try to find it
            ...
            for name in game_names:
                ...
                if FIELD_CUSTODIAN in idx:
                    ...
                    if custodian:
                        ...
                        if custodian_member:
                            add_update(row_idx, idx[FIELD_BORROWER_ID], custodian_member.get('id', ''))
    ...
    def batch_return_games_by_member(self, member_id: str) -> Tuple[bool, str, List[str]]:
        ...
        try:
            ...
            borrowed_games = [
                g for g in games 
                if g.get(FIELD_STATUS) == GAME_STATUS_BORROWED and g.get(FIELD_BORROWER) == member_name
            ]
