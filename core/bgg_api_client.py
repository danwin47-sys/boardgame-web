...
class BGGApiClient:
    ...
    def __init__(self, api_token: str = "", timeout: int = 15, retries: int = 2):
        """
        初始化 BGG API 客戶端
        ...
        if self.api_token:
            ...
            logger.info("BGG API Client initialized with Bearer Token and User-Agent")
        else:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                if response.status_code == 401:
                    ...
                    logger.error(f"BGG API returned status {response.status_code}")
                    return None
    ...
    def game(self, game_id: int) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            桌遊詳細資訊字典
        """
        params = {"id": game_id, "stats": "1"}  # 包含統計資料（評分、排名等）
