# 🗺️ Boardgame-Web 開發路線圖

> 最後更新：2025-12-17

## 📊 專案現況

**當前版本：** v3.2 (搜尋功能重構 + Mypy 類型檢查完成)

### ✅ 已完成功能

| 模組 | 功能 | 狀態 |
|------|------|------|
| **桌遊管理** | 借還操作、狀態追蹤、歷史記錄 | ✅ 完成 |
| **BGG 整合** | 遊戲連結、搜尋、熱門排行、推薦 | ✅ 完成 |
| **BGG Ranks** | 離線資料庫、30萬+遊戲查詢 | ✅ 完成 |
| **Gallery Wall** | 視覺化展示、多條件過濾 | ✅ 完成 |
| **管理後台** | 批次操作、權限控管 | ✅ 完成 |
| **會員管理** | 搜尋、借閱記錄 | ✅ 完成 |
| **成就系統** | 成就追蹤、視覺化呈現 | ✅ 完成 |
| **Email 通知** | 關鍵更新通知 | ✅ 完成 |
| **BGG 加入** | 從 BGG 搜尋並加入遊戲到館藏 | ✅ 完成 |

---

## 🎯 v3.2 - 使用者體驗優化 (短期目標)

**預計完成：** 2025 Q1

### 🔧 改進項目

- [x] **搜尋強化** *(已完成 - 全站搜尋 + 模糊匹配)*
  - [x] 全站搜尋功能（遊戲 + 會員）
  - [x] 搜尋歷史與建議 *(已完成 - localStorage + 自動完成)*
  - [x] 拼音/注音模糊搜尋（部分完成 - 相似度匹配）

- [x] **UI/UX 改進** *(已完成)*
  - [x] 深色模式支援
  - [x] 行動裝置手勢操作
  - [x] 載入動畫與骨架屏
  - [x] 更豐富的遊戲卡片資訊

- [x] **效能優化** *(已完成)*
  - [x] 圖片懶加載
  - [x] API 回應快取優化
  - [x] 前端資源壓縮

---

## 🚀 v3.3 - 智慧功能 (中期目標)

**預計完成：** 2025 Q2

### 🤖 新功能

- [ ] **智慧推薦系統**
  - 基於借閱歷史的個人化推薦
  - 「類似遊戲」推薦
  - 季節性/節日推薦

- [ ] **統計儀表板**
  - 借閱趨勢圖表
  - 熱門遊戲排行
  - 會員活躍度分析
  - 庫存狀態總覽

- [ ] **通知系統強化**
  - LINE / Discord 機器人通知
  - 借閱逾期自動提醒
  - 新遊戲到館通知

- [ ] **進階擴充管理 (Expansion Management)**
  - 支援「合併收納」與「獨立收納」兩種模式
  - 合併收納：借出主遊戲時自動標記擴充為借出
  - 獨立收納：借出擴充時提示是否需要主遊戲
  - 介面優化：主遊戲與擴充的層級顯示

---

## 🌟 v4.0 - 社群與互動 (長期目標)

**預計完成：** 2025 Q3-Q4

### 🎮 大型功能

- [ ] **遊戲評分與評論**
  - 會員評分功能
  - 遊戲評論/心得
  - 評分排行榜

- [ ] **活動系統**
  - 桌遊聚會活動管理
  - 活動報名與簽到
  - 活動照片記錄

- [ ] **願望清單**
  - 會員想玩清單
  - 社團採購建議
  - BGG 願望清單同步

- [ ] **多語言支援**
  - 英文介面
  - 遊戲名稱多語言顯示

---

## 🔒 技術債與維護

### 持續性任務

- [x] 提升測試覆蓋率至 80% *(已達成 81%，共 256 測試)*
- [x] API 文件自動生成 (OpenAPI/Swagger) *(已完成 - /api/docs)*
- [x] 日誌系統完善 *(已完成 - core/logging_config.py)*
- [x] 容器化部署優化 (Docker Compose) *(已完成 - docker-compose.yml)*
- [x] CI/CD 流程完善 (GitHub Actions) *(已完成 - .github/workflows/ci.yml)*

### 程式碼品質

- [x] 移除 `boardgame_system.py` 舊架構 *(已重構為 `core/facade.py`)*
- [x] 統一錯誤處理機制 *(已完成 - error_code, to_dict())*
- [x] Pydantic 模型導入 *(已完成 - app/schemas.py)*
- [x] 類型檢查 (mypy) 通過率 100% *(已完成 - 2025-12-17)*

---

## 📅 里程碑時間表

```mermaid
gantt
    title Boardgame-Web 開發時程
    dateFormat  YYYY-MM
    section v3.2
    搜尋強化           :2025-01, 2025-02
    UI/UX 改進         :2025-01, 2025-03
    效能優化           :2025-02, 2025-03
    section v3.3
    智慧推薦系統       :2025-03, 2025-05
    統計儀表板         :2025-04, 2025-06
    通知系統強化       :2025-05, 2025-06
    section v4.0
    遊戲評分與評論     :2025-07, 2025-09
    活動系統           :2025-08, 2025-10
    願望清單           :2025-09, 2025-11
    多語言支援         :2025-10, 2025-12
```

---

## 🙋 如何貢獻

1. 從上方選擇一個感興趣的功能
2. 在 GitHub Issues 中認領任務
3. 建立功能分支開發
4. 提交 Pull Request

---

## 📝 變更記錄

| 版本 | 日期 | 主要變更 |
|------|------|----------|
| v3.2 | 2025-12-17 | 搜尋功能重構、Mypy 類型檢查完成 |
| v3.1 | 2025-12-16 | 技術債處理、Docker Compose、API 文件 |
| v3.0 | 2025-12-04 | Gallery Wall、BGG Ranks 整合 |
| v2.0 | 2025-11 | BGG 整合、成就系統 |
| v1.0 | 2025-10 | 基礎借還功能 |
