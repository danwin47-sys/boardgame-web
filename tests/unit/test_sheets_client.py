...
from core.constants import GAME_STATUS_AVAILABLE, GAMES_CACHE_TTL, MEMBERS_CACHE_TTL
...
class TestSheetsClientLoadData:
    ...
    def test_load_games_retry(self):
        """測試讀取失敗後重試"""
        # 第一次呼叫失敗，第二次成功 (模擬重連後)
        self.mock_ws.get_all_records.side_effect = [
            Exception("API Error"),
            [{"name": "Catan"}],
        ]
    ...
    def test_load_members_retry(self):
        """測試讀取會員失敗後重試"""
        # 第一次呼叫失敗，第二次成功 (模擬重連後)
        self.client.members_ws.get_all_records.side_effect = [
            Exception("API Error"),
            [{"id": "001"}],
        ]
...
class TestSheetsClientWriteData:
    ...
    def test_update_game_bgg_id_success(self):
        ...
        self.client.games_ws.row_values.return_value = [
            "name",
            "bgg_id",
            "bgg_thumbnail",
            "image",
...
class TestSheetsClientGameUpdates:
    ...
    def test_update_game_playtime_success(self):
        ...
        self.mock_games_ws.row_values.return_value = [
            "name",
            "minplaytime",
            "maxplaytime",
        ]
