let bggSearchTimeout=null;const bggCategoryCache={};function initBGG(){const bggSearchBtn=document.getElementById('bggSearchBtn');if(bggSearchBtn){bggSearchBtn.addEventListener('click',()=>openBGGSearchModal());}
preloadAllData();}
function openBGGSearchModal(){const modal=document.createElement('div');modal.className='bgg-modal';modal.id='bggSearchModal';modal.innerHTML=`<div class="bgg-modal-content bgg-search-modal-content"><span class="bgg-modal-close"onclick="closeBGGSearchModal()">&times;</span><h2>🔍 搜尋 BoardGameGeek 桌遊</h2><div class="bgg-search-box"><div class="search-input-group"><input type="text"id="bggSearchBoxModal"class="search"placeholder="輸入桌遊名稱（中文或英文）..."><button class="btn primary"onclick="searchBGGInModal()">搜尋</button></div><p class="search-hint">💡 提示：輸入至少 3 個字元會自動搜尋</p></div><div class="bgg-search-results"><div id="bggSearchResultsModal"class="bgg-results-list"></div></div></div>`;document.body.appendChild(modal);const searchBox=document.getElementById('bggSearchBoxModal');if(searchBox){searchBox.addEventListener('keypress',(e)=>{if(e.key==='Enter'){searchBGGInModal();}});searchBox.addEventListener('input',()=>{clearTimeout(bggSearchTimeout);const query=searchBox.value.trim();if(query.length>=3){bggSearchTimeout=setTimeout(()=>searchBGGInModal(),500);}});searchBox.focus();}}
function closeBGGSearchModal(){const modal=document.getElementById('bggSearchModal');if(modal){modal.remove();}}
async function searchBGGInModal(){const query=document.getElementById('bggSearchBoxModal').value.trim();if(!query){showToast('請輸入搜尋關鍵字','error');return;}
const resultsDiv=document.getElementById('bggSearchResultsModal');resultsDiv.innerHTML='<p class="loading">正在搜尋...</p>';try{const response=await fetch(`/api/bgg/search?q=${encodeURIComponent(query)}`);const data=await response.json();if(data.success&&data.results&&data.results.length>0){displayBGGResultsInModal(data.results);}else{resultsDiv.innerHTML='<p class="no-results">找不到相關桌遊，請嘗試其他關鍵字</p>';}}catch(error){console.error('BGG search error:',error);resultsDiv.innerHTML='<p class="error">搜尋時發生錯誤，請稍後再試</p>';}}
function displayBGGResultsInModal(results){const resultsDiv=document.getElementById('bggSearchResultsModal');resultsDiv.innerHTML='<h3>搜尋結果</h3>';results.forEach(game=>{const card=createBGGGameCard(game);resultsDiv.appendChild(card);});}
function displayBGGResults(results){const resultsDiv=document.getElementById('bggResults');const resultsList=document.getElementById('bggResultsList');if(!results||results.length===0){resultsDiv.style.display='none';showToast('沒有找到相關桌遊','info');return;}
resultsList.innerHTML='';results.forEach(game=>{const card=createBGGGameCard(game);resultsList.appendChild(card);});resultsDiv.style.display='block';}
function createBGGGameCard(game){const card=document.createElement('div');card.className='bgg-game-card';card.innerHTML=`<div class="bgg-game-info"><h4>${game.name}</h4><p class="bgg-game-year">${game.year||'N/A'}</p></div><div class="bgg-game-actions"><button class="btn small"onclick="viewBGGGameDetails(${game.id})">查看詳情</button><button class="btn small primary"onclick="addBGGGameToCollection(${game.id}, '${game.name}')">加入館藏</button></div>`;return card;}
function viewBGGGameDetails(gameId,gameName=null){fetch(`/api/bgg/games/${gameId}`).then(r=>r.json()).then(data=>{if(data.success){showBGGGameModal(data.game,gameName);}else{alert('無法載入遊戲詳情');}}).catch(err=>{console.error('Error fetching game details:',err);alert('載入失敗');});}
function showBGGGameModal(game,linkedGameName=null){const footerButtons=linkedGameName?`<button class="btn"onclick="closeBGGModal()">關閉</button><button class="btn danger"onclick="unlinkGameFromBGG('${linkedGameName.replace(/'/g, "\\'")}')">斷開連結</button>`
