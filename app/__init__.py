...
def create_app(config_name=None):
    ...
    def add_cache_headers(response):
        ...
        # 靜態資源（CSS, JS, 圖片）
        if request.path.startswith('/static/') or request.path.startswith('/css/') or request.path.startswith('/js/'):
            # 1 年快取
            response.cache_control.max_age = 31536000
            response.cache_control.public = True
            
            # 添加 ETag 支援
            if response.data and len(response.data) < 1024 * 1024:  # 只為 < 1MB 的檔案生成 ETag
